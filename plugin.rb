...
after_initialize do
  DiscourseEvent.on(:topic_created) do | topic, options, user |
    Rails.logger.debug("Topic created, maybe notifying, enabled: #{SiteSetting.new_topic_notification_enabled} notifyinguser: #{SiteSetting.new_topic_notifying_user}")
        ...
        if user_notifying_new_post then
          ...
          if new_topic_category_id then
            ...
            Topic.where(category_id: new_topic_category_id).recent(2) do | existing_topic |
              Rails.logger.debug("Checking if topic #{existing_topic_id} matches #{topic.id}")
              if existing_topic.id != topic.id
                last_topic = existing_topic
              end
            end
            if last_topic then
              Rails.logger.debug("Last topic: #{last_topic.title}")
              topic_embed = TopicEmbed.where(topic_id: topic.id).first
              if topic_embed then
                Rails.logger.debug("Topic embed: #{topic_embed}")
                ...
                Rails.logger.debug("Could not find topic embed for topic id: #{topic.id}")
              end
            else
              Rails.logger.debug("Could not find previous topic in category: #{SiteSetting.new_topic_notification_category}")
            end
          else
            Rails.logger.debug("Could not find matching category for topics to notify against: #{SiteSetting.new_topic_notification_category}")
          end
        else
          Rails.logger.debug("Could not find matching user to assign as notifier: #{SiteSetting.new_topic_notifying_user}")
        end
      end
  end
end